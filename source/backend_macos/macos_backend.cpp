...
namespace lightphi::input
{
    ...
    class InputSourceImplementation final : public apple_detail::IDeliveryTarget
    {
      public:
        ...
        // initialized first.
        InputSourceImplementation(const std::uint32_t queueCapacity, const bool acceptPointerFallback) noexcept
            : Core{queueCapacity, gsl::not_null<apple_detail::IDeliveryTarget *>{this}},
              Sink{gsl::not_null<apple_detail::IEventTarget *>{&Core}},
              Samples{new (std::nothrow) InputSample[queueCapacity]{}}, AcceptPointerFallback{acceptPointerFallback}
        {
        }
        ...
        [[nodiscard]] bool StartCapture() noexcept
        {
            ...
            Monitor = LightPHIMacOSCapture::createAppleInputMonitor(&Sink, AcceptPointerFallback);
    ...
    CreateInputSource(const InputSourceDescriptor &descriptor) noexcept
    {
        ...
        InputSource::Implementation implementation{new (std::nothrow) InputSourceImplementation{
            descriptor.MaximumQueuedBatches, descriptor.AcceptPointerFallback}};
