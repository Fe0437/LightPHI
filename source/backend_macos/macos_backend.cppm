...
export namespace lightphi::input
{
    ...
    /** {brief} Fixed storage and device policy selected before macOS capture starts. */
    struct InputSourceDescriptor
    {
        std::uint32_t MaximumQueuedBatches{64U}; ///< Deliveries retained while the receiver is busy.

        /**
         * {brief} Whether an ordinary pointer may draw when no pen is present.
         *
         * Off by default, because this interface is about pen hardware and a caller asking for a
         * pen should not silently be handed a mouse. An application whose window must stay usable
         * on a machine with no tablet turns it on and receives real pointer contacts, reported
         * with the capabilities that device actually has - no pressure, tilt, twist or eraser.
         * Nothing is synthesized: a value that was never measured is never invented.
         *
         * ```cpp
         * // A drawing application that must work before a tablet is plugged in.
         * auto source{CreateInputSource({.AcceptPointerFallback = true})};
         * ```
         */
        bool AcceptPointerFallback{false};
    };
